...
export class EstablishmentsController {
  ...
  async create(@Body() createEstablishmentDto: CreateEstablishmentDto) {
    const establishment = await this.establishmentsService.create(createEstablishmentDto);
    return 'inserted'
  }
  ...
  @Get()
  async findAll(@Request() req: Request, @Query('page') page?: number, @Query('limit') limit?: number) {
    const options = {
      page: +(page || 1),
      limit: +(limit || 10),
      route: req.url
  ...
  @Patch(':id')
  async update(@Param('id', ParseIntPipe) id: number, @Body() updateEstablishmentDto: UpdateEstablishmentDto) {
  ...
  async insertVehicle(
    ...
    if( establishment.vehicles.find(vehicle => vehicle.board === insertCarEstablishment.board) ) {
    ...
    const vehicle = await this.vehicleService.findByBoard(insertCarEstablishment.board)
    const establishmentVehicles = establishment.vehicles.map(v => v.vehicleType === vehicle.vehicleType)
    ...
    if( establishmentVehicles.length >= establishment.getVacancies(vehicle.vehicleType) ) {
      throw new BadRequestException(
          'Establishment is full',
    ...
    return await AppDataSource.transaction(async transactionalEntityManager => {
      ...
      const movement = AppDataSource.getRepository(EstablishmentMovement).create({
        vehicle: vehicle,
  ...
  async removeVehicle(
    ...
    if( ! establishment.vehicles.find(vehicle => vehicle.board === removeCarEstablishment.board) ) {
      throw new BadRequestException(
          'Vehicle not found at this establishment',
          {description: 'ESTABLISHMENT_VEHICLE_NOT_FOUND'}
      )
    }

    const vehicle = await this.vehicleService.findByBoard(removeCarEstablishment.board)
  ...
  async getMovement(
    ...
    if(dateStart >= dateEnd) {
      throw new BadRequestException(
          'Date start not be >= to the date end.',
