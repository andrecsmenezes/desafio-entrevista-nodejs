...
import {DeleteResult, EntityNotFoundError, FindOptionsWhere, Repository, UpdateResult} from "typeorm";
...
export class EstablishmentsService {
  ...
  async create(createEstablishmentDto: CreateEstablishmentDto): Promise<Establishment> {
    ...
    const address = this.addressRepository.create(createEstablishmentDto.address);

    return await AppDataSource.transaction(async transactionalEntityManager => {
      const addressTransaction = await transactionalEntityManager.save(address);
  ...
  async findOne(id: number): Promise<Establishment> {
    try {
      return await this.establishmentRepository.findOneOrFail({where: {id}, relations: {address: true, vehicles: true}});
    }
    catch(error) {
      if(error instanceof EntityNotFoundError) {
        throw new BadRequestException('Establishment not found', {description: 'ESTABLISHMENT_NOT_FOUND'})
      }

      this.logger.error(error)
      throw new BadRequestException('Something as wrong', {description: 'ESTABLISHMENT_UNKNOWN_ERROR'})
    }
  }

  async update(id: number, updateEstablishmentDto: UpdateEstablishmentDto): Promise<UpdateResult> {
    await this.establishmentRepository.findOneOrFail({where: {id}});
    return await this.establishmentRepository.update(id, updateEstablishmentDto);
  }
  ...
  async getMovement(options: IPaginationOptions, findOptions: any) {
    return await paginate<EstablishmentMovement>(this.establishmentMovementRepository, options, findOptions);
  }

  async getMovementPerHour(id: number, options: IPaginationOptions) {
    const queryBuilder = this.establishmentMovementRepository.createQueryBuilder();
    queryBuilder
        ...
        .groupBy('YEAR(dateTime), MONTH(dateTime), DAY(dateTime), HOUR(dateTime), movementType')
        .orderBy('YEAR(dateTime), MONTH(dateTime), DAY(dateTime), HOUR(dateTime), establishmentId, movementType, COUNT(1)', 'DESC');

    const result = await paginateRaw<EstablishmentMovement>(queryBuilder, options);
