...
export class VehiclesService {
  ...
  async update(id: number, updateVehicleDto: UpdateVehicleDto): Promise<UpdateResult> {
  ...
  async findByBoard(board: string): Promise<Vehicle> {
    try {
      return await this.vehicleRepository.findOneOrFail({where: {board}, relations: ['establishment']})
    }
    catch (e) {
      if(e instanceof EntityNotFoundError) {
        throw new BadRequestException('Vehicle not found', {description: 'VEHICLE_NOT_FOUND'});
      }

      throw new BadRequestException('Something was wrong on search vehicle', {description: 'VEHICLE_UNKNOWN_ERROR'});
    }
  }
}
